...
class BasePageLogin(BasePage):
    ...
    def login(cls):
        """登录"""
        with allure.step(f"{cls.page_name}-接受用户条款"):
            find_node_until_end(["jieshou"])
            poco(text = "接受").click()
            cls.take_step_screenshot("接受用户条款")

        with allure.step(f"{cls.page_name}-同意用户协议"):
        ...
        with allure.step(f"{cls.page_name}-切换邮箱登录"):
            find_node_until_end(["email"])
            poco(desc="IM30邮箱").click()
            cls.take_step_screenshot("切换邮箱登录")
            
        with allure.step(f"{cls.page_name}-填写邮箱账号"):
        ...
        with allure.step(f"{cls.page_name}-填写邮箱密码"):
        ...
        with allure.step(f"{cls.page_name}-点击登录"):
