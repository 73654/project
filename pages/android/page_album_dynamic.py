...
class AndroidBasePageAlbumDynamic(BasePageAlbumDynamic):
    ...
    def page_check_commodity_info(cls):
        with dog.step(f"{cls.page_name}-检查新增商品价格"):
            sleep(ui.step_wait_time)
            assert_is_not_none(find_area_image(Template(r"tpl1745733927175.png"), target_rect=(get_vertical_rect(-0.35))))
            assert_is_not_none(find_area_image(Template(r"tpl1745733939543.png"), target_rect=(get_vertical_rect(-0.2))))
