...
class DogCheck:
    ...
    def not_equal(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} 期望值不等于：{expect} 实际值：{act}")
    ...
    def is_(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 是 {expect}")
    ...
    def is_not(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 不是 {expect}")
    ...
    def is_in(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 在 {expect} 中")
    ...
    def is_not_in(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 不在 {expect} 中")
    ...
    def is_instance(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 是 {expect} 的实例")
    ...
    def is_not_instance(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 不是 {expect} 的实例")
    ...
    def almost_equal(act, expect, rel=None, abs=None, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 近似等于 {expect} (rel={rel}, abs={abs})")
    ...
    def not_almost_equal(act, expect, rel=None, abs=None, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 不近似等于 {expect} (rel={rel}, abs={abs})")
                assert act != pytest.approx(expect, rel=rel,
    ...
    def greater(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 大于 {expect}")
    ...
    def greater_equal(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 大于等于 {expect}")
    ...
    def less(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 小于 {expect}")
    ...
    def less_equal(act, expect, title=""):
        with check():
            with dog.step(title):
                log.info(f"检查: {title} {act} 小于等于 {expect}")
